...
《**終噬虛空**》整體來說是個非常輕量的遊戲，只要你的裝置至少比 Steam Deck 還強的話就不應該有問題。
...
11. 除此之外，確保你的 Windows 效能設定在執行《**終噬虛空**》時是處於「高效能模式」。確保遊戲執行的螢幕是連接在 Nvidia 顯示卡上的。
...
Windows：
`使用者\[Username]\AppData\LocalLow\ChosenConcept\UltraVoid\Player.log`
或直接在檔案總管位址欄輸入：`%userprofile%\AppData\LocalLow\ChosenConcept\UltraVoid\`就可以到這個資料夾，再提供裡面的 Player.log
