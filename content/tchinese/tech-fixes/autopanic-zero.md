...
由於《自動混亂：零式》的大量敵人是遊戲特色，實測中可以配合動態解析度穩定運行在 60FPS，但 90FPS 就比較困難了，因此對於 Steam Deck OLED 的玩家建議用系統鎖在 60FPS。不過如果真的很想要高幀率遊玩，啟用「顯示」→「進階」→「CPU 輕量模式」能在部分犧牲視覺表現的情況下大幅提升幀率。

目前 Unity 引擎與 Steam Deck OLED 的幀率鎖定有一些配合問題，直接使用遊戲內的幀率鎖定機制會鎖定在奇怪的 68FPS，想要超過 60FPS 的時候請在遊戲內解除幀率鎖定，單純靠 Steam Deck 的系統進行鎖定。
