...
Steam Deck 上遊玩《自動混亂：零式》要透過 Proton 遊玩 Windows 版，之所以如此是因為測試中 Unity 引擎原生輸出的 Linux 版效能更糟，所以透過 Proton 模擬的體驗反而比較好。

由於《自動混亂：零式》的大量敵人是遊戲特色，實測中可以配合動態解析度穩定運行在 60FPS，但 90FPS 就比較困難了，因此對於 Steam Deck OLED 的玩家建議用系統鎖在 60FPS。不過如果真的很想要高幀率遊玩的話，啟用「顯示」→「進階」→「CPU 輕量模式」能在部分犧牲視覺表現的情況下大幅提升幀率。

目前 Unity 引擎與 Steam Deck OLED 的幀率鎖定有一些配合問題，直接使用遊戲內的幀率鎖定機制會可能會鎖定在奇怪的 68FPS，如遭遇此狀況且想要超過 60FPS 的時候請在遊戲內解除幀率鎖定，單純靠 Steam Deck 的系統進行鎖定。

Steam Deck 初次遊玩時，系統會預設為以下選項：

- 將破壞模擬設定為「關閉」
- 將動態光線設定為「關閉」
- 啟用動態解析度：將目標幀率設定在 90FPS、將解析度下限設定在 0.5 倍
- 啟用 CPU 輕量模式
- 關閉幀率鎖定

之所以預設動態解析度直接設定在 90FPS，是因為 Steamworks 可以偵測是 Steam Deck 但沒辦法偵測是不是 OLED 機種，為了確保玩家立刻有較好的體驗所以設定得比較激進。
...
- 在「顯示」→「進階」關閉「破壞模擬」
- 在「顯示」→「進階」關閉「動態光源」
- 在「顯示」→「進階」啟用「動態解析度」和／或「FSR2」、「DLSS」相關設定

雖然一般來說降低解析度不會影響 CPU，但由於本遊戲的渲染性質，降低解析度可以相當程度緩解 CPU 負載。除此之外，雖然會影響到畫面表現，但若想要進一步降低整體負載的話，可以這麼做：

- 在「顯示」→「進階」啟用「CPU 輕量模式」

而如果前述選項都沒有幫助，而你認為你的裝置比最低測試過的還要好，可以依序嘗試以下操作：
