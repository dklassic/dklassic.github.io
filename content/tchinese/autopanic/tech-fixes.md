...
由於《自動混亂》基本上是以 Steam Deck 為基礎設計的，所以可以保證 Steam Deck 能以 800p/90FPS 穩定運行。
...
《***自動混亂***》使用了一套對<abbr title="CPU">中央處理器</abbr>需求極低的自訂框架，所以如果你遭遇的是 CPU 效能瓶頸，很遺憾並沒有太多可以調整的空間。

若是遭遇到 GPU 效能瓶頸，以下選項可能有所幫助：
...
如果你的存檔消失或者可能損毀而沒有辦法正確載入遊戲，請試著重新安裝遊戲。如果依然無效的話，請參照以下步驟：

- 在 **Windows** 上，前往「\文件\Saved Games\Autopanic」
- 在 **macOS** 上，前往「/Users/[UserName]/Library/Application Support/ChosenConcept/Autopanic/Save/」
...
2. 根據**修改日期**排序，並且找到最新的備份存檔
	...
	- 如果存檔損毀，請使用「save.dat.bak0」，這是關卡開始時的備份存檔，應該一定可以使用。
